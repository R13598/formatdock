    ...
    <div className="mx-auto max-w-6xl">
      <div className="rounded-2xl border border-border bg-card p-6 sm:p-8">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-lg font-bold text-foreground">Make it Look Scanned</h2>
            <p className="text-xs text-muted-foreground">Transform a clean PDF into a realistic scanned document</p>
          </div>
        </div>
        ...
        {!file ? (
          ...
          >
            ...
            <p className="mt-1.5 text-xs text-muted-foreground">We&apos;ll make it look like it came straight from a scanner</p>
            <div className="mt-3 flex flex-wrap items-center justify-center gap-1.5">
              <span className="rounded-md border border-border bg-muted/60 px-2 py-0.5 font-mono text-[10px] text-muted-foreground">PDF</span>
            </div>
            <input
              id="scan-file-input"
              type="file"
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_300px]">
            ...
            <div className="flex flex-col gap-5 rounded-xl border border-border bg-muted/20 p-5">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-foreground">Grayscale / B&amp;W</p>
                  <p className="text-[11px] text-muted-foreground">Remove all color</p>
                </div>
                <button
                  onClick={() => setGrayscale((v) => !v)}
                  className={cn(
                    'relative h-6 w-11 rounded-full transition-colors duration-200',
                    grayscale ? 'bg-primary' : 'bg-muted-foreground/30'
                  )}
                >
                  <span
                    className={cn(
                      'absolute top-0.5 h-5 w-5 rounded-full bg-white transition-transform duration-200 shadow',
              ...
              <div>
                <div className="flex items-center justify-between">
                  <p className="text-sm font-medium text-foreground">Contrast Booster</p>
              ...
              {/* Grain & blur slider */}
              <div>
                <p className="text-sm font-medium text-foreground">Scanner Grain &amp; Blur</p>
                  ...
                  {(['low', 'medium', 'high'] as GrainLevel[]).map((level) => (
                    <button
                      ...
                      className={cn(
                        ...
                        grain === level
                          ? 'border-primary bg-primary/15 text-primary shadow-[0_0_12px_-4px_rgba(37,99,235,0.5)]'
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-foreground">Imperfect Page Tilt</p>
                  <p className="text-[11px] text-muted-foreground">Random -0.6° to +0.6° rotation</p>
                </div>
                <button
                  onClick={() => setTilt((v) => !v)}
                  className={cn(
                    'relative h-6 w-11 rounded-full transition-colors duration-200',
                    tilt ? 'bg-primary' : 'bg-muted-foreground/30'
                  )}
                >
                  <span
                    className={cn(
              ...
              {busy && (
                <div>
                  <div className="mb-1 flex justify-between text-xs">
                    <span className="text-muted-foreground">Processing page {Math.ceil((progress / 100) * pageCount)} of {pageCount}</span>
              ...
              <div className="flex flex-col gap-2">
                ...
                {resultUrl && (
                  <a href={resultUrl} download="formatdock-scanned.pdf">
                    <button className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-emerald-500/40 bg-emerald-500/10 px-5 py-2.5 text-sm font-semibold text-emerald-600 dark:text-emerald-400 transition-colors hover:bg-emerald-500/20">
                ...
                <button
                  onClick={reset}
                  disabled={busy}
                  className="inline-flex items-center justify-center gap-2 rounded-lg border border-border bg-background px-4 py-2.5 text-sm font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground disabled:opacity-50"
                >
