...
export default function PdfCompressor() {
  ...
  const compress = useCallback(async () => {
    ...
    try {
      ...
      // Re-save with object stream compression and remove metadata to shrink size.
      // For image-heavy PDFs we re-encode embedded JPGs at lower quality.
      const newPdf = await PDFDocument.create();
      const copied = await newPdf.copyPages(pdf, pages.map((_, i) => i));
      copied.forEach((p) => newPdf.addPage(p));

      // Try progressively lower compression levels until target met or floor reached.
      let outBytes = await newPdf.save({ useObjectStreams: true });
      const targetBytes = targetKb * 1024;

      if (outBytes.length > targetBytes) {
        // Further compression: strip metadata and re-save with object streams
        newPdf.setTitle('');
        newPdf.setAuthor('');
        newPdf.setSubject('');
        newPdf.setKeywords([]);
  ...
  return (
    <div className="max-w-2xl rounded-2xl border border-[#1e293b] bg-[#0f172a] p-6 sm:p-8">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-lg font-bold text-white">PDF Compressor</h2>
      </div>
      ...
      {file && (
        <div className="mt-4 rounded-lg border border-[#1e293b] bg-[#020617] p-4">
          <div className="flex items-center justify-between">
            <div className="min-w-0">
              <p className="truncate text-sm font-medium text-slate-200">{file.name}</p>
              <p className="text-xs text-slate-500">Original: {(origSize / 1024).toFixed(0)} KB</p>
            </div>
          ...
          <div className="mt-4 flex items-center gap-3">
            <label className="text-xs font-semibold uppercase tracking-wide text-slate-400">Target size</label>
            <div className="flex items-center gap-2">
              <input
                type="number"
                min={20}
                ...
                className="w-24 rounded-lg border border-[#1e293b] bg-[#020617] px-3 py-2 text-sm text-white focus:border-[#2563EB] focus:outline-none focus:ring-2 focus:ring-[#2563EB]/30"
              />
