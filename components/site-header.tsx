...
const categoryTabs = [
  { href: '/exams', label: 'Exam Resizers', icon: '🎓' },
  { href: '/documents', label: 'Document Utilities', icon: '📄' },
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center gap-3 px-4 sm:px-6 lg:px-8">
        {/* Brand Logo - Uses text-foreground for dynamic light/dark support */}
        <Link href="/" className="group flex shrink-0 items-center gap-2.5 mr-2">
        ...
        <nav className="hidden items-center gap-1 xl:flex">
          ...
          >
            <Link
              href="/tools"
              className={cn(
                "flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold text-muted-foreground transition-all duration-200 hover:bg-muted hover:text-foreground",
            ...
            {megaOpen && (
              <div
                className="absolute left-0 top-full z-50 w-[880px] pt-3 animate-fade-in-up"
                onMouseEnter={openMega}
                onMouseLeave={closeMega}
              >
                <div className="overflow-hidden rounded-2xl glass-panel shadow-2xl border border-border/80 bg-card/95">
                  ...
                  <div className="border-t border-border/60 bg-muted/40 px-5 py-3 flex items-center justify-between">
                    <span className="text-xs text-muted-foreground">
                      ⚡ 33+ Utilities • Zero Server Uploads • 100% Private
                    </span>
                    <Link
                      href="/tools"
                      onClick={() => setMegaOpen(false)}
          ...
          {categoryTabs.map((tab) => (
            <Link
              key={tab.href}
              href={tab.href}
              className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-medium text-muted-foreground transition-all duration-200 hover:bg-muted hover:text-foreground"
            >
              <span>{tab.icon}</span>
              <span>{tab.label}</span>
            </Link>
        ...
        <div className="relative ml-auto hidden max-w-xs flex-1 md:block">
          ...
          <Input
            ...
            onKeyDown={onKeyDown}
            className="h-9 border-border/80 bg-card/60 pl-8 pr-12 text-xs text-foreground placeholder:text-muted-foreground search-glow focus-visible:ring-primary/30"
          ...
          {searchOpen && suggestions.length > 0 && (
            <div className="absolute left-0 right-0 top-11 z-50 overflow-hidden rounded-xl glass-panel shadow-2xl border border-border bg-card/95 animate-fade-in-up">
              {suggestions.map((t, i) => (
                <Link
                  key={t.slug}
                  href={`/tools/${t.slug}`}
        ...
        <button
          className="ml-auto flex h-9 w-9 items-center justify-center rounded-lg border border-border bg-card text-foreground transition-colors hover:bg-muted xl:hidden"
          onClick={() => setOpen((o) => !o)}
      ...
      {/* Mobile Drawer */}
      {open && (
        <div className="border-b border-border bg-background/98 backdrop-blur-xl xl:hidden animate-fade-in-up">
