  ...
  return (
    <div className="max-w-2xl rounded-2xl border border-border bg-card p-6 sm:p-8">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-lg font-bold text-foreground">Image to PDF Converter</h2>
      </div>
      ...
      >
        ...
        <p className="mt-1.5 text-xs text-muted-foreground">JPG, PNG — multiple files supported</p>
      ...
      {images.length > 0 && (
        <div className="mt-4 flex flex-wrap gap-2">
          {images.map((img, i) => (
            <div key={i} className="h-16 w-16 overflow-hidden rounded border border-border bg-card">
              <img src={img.src} alt="" className="h-full w-full object-cover" />
            </div>
          ))}
          <button
            onClick={() => inputRef.current?.click()}
            className="flex h-16 w-16 items-center justify-center rounded border border-dashed border-primary/40 text-primary transition-colors hover:bg-primary/10"
          >
